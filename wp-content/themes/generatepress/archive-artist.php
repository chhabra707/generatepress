...
<?php
get_header(); 
	//prepare form fields and data
	$actionUrl = home_url( $wp->request );
	$postType  = array('artist');
	$taxonomy  = array('artist_cat');
  ...
  <div class="wrapper">
    <div id="category_research" class="clear_both"> 
      <form id="searchForm" action="<?php echo $actionUrl; ?>" method="post" class="clear_both">
		...
		<?php endforeach; ?>
		<?php wp_nonce_field( 'all_artists_filter', 'allArtists' ); ?>		
...
?>									
	...
	//$postType = "'" . implode ( "', '", $postType ) . "'";
		...
		if (isset( $_POST['allArtists'] ) &&  wp_verify_nonce( $_POST['allArtists'], 'all_artists_filter' ) ):
...
/* 			
			if(!empty($_postArr) && is_array($_postArr)){
				$_postStr = "'" . implode ( "', '", $_postArr ) . "'";
				$filters[] =	" ( wp.ID IN($_postStr))";				
			}
				//$args['p'] = $_postArr; 
			if(!empty($_category) && is_array($_category)){
				$_categoryStr = "'" . implode ( "', '", $_category ) . "'";
				$filters[] =	" ( wt.term_id IN ($_categoryStr) )";				
			}
			...
			if(is_array($filters) AND !empty($filters)){
				...
				//$args['tax_query'] = array('taxonomy' => $taxonomy,'field' => 'term_id','terms' => $_category);
		endif; */
