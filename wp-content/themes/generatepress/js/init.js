...
$(function(){
    if($('#interactive_map').length){		
        ...
        //interactive_map_init(artists,'arts',false);
		//interactive_map_init(saveurs,'saveurs',false);
...
function interactive_map_init(arr,div,type){
	if(type === false){
		...
		for(var i=0;i<arr.length;i++){
			var subcat_post_count = 0;
			var maincat = arr[i];
			$('#interactive_map_cat .select_items.'+div+' > ul').append('<li><div>'+maincat['maincategory']+'</div></li>');
			$('#interactive_map_cat .select_items.'+div+' > ul > li:last-of-type').append('<ul>');
			for(var j=0;j<maincat['subcategories'].length;j++){
				var subcat = maincat['subcategories'][j];
				$('#interactive_map_cat .select_items.'+div+' > ul > li:last-of-type > ul').append('<li class="item"><div>'+subcat['subcategory']+'<span class="dv-subcat-numb">('+subcat['posts'].length+')</span></div></li>');
				$('#interactive_map_cat .select_items.'+div+' > ul > li:last-of-type > ul > li:last-of-type').append('<ul>');
				for(var k=0;k<subcat['posts'].length;k++){
					subcat_post_count++;
					post_count++;
					
					var post = subcat['posts'][k];
					//console.log(post['page']);
					$('#interactive_map_cat .select_items.'+div+' > ul > li:last-of-type > ul > li:last-of-type > ul').append(
						'<li>'
						+'<div data-url="'+post['url']+'" data-map_town="'+post['map_town']+'" data-number="'+post['number']+'" data-town="'+post['town']+'" data-phone_second="'+post['phone_second']+'" data-email_second="'+post['email_second']+'" data-url="'+post['page']+'" data-marker="'+post['mapPin']+'" data-email="'+post['email']+'" data-address="'+post['address']+'" data-phone="'+post['phone']+'" data-image="'+post['featuredImage']+'" data-lng="'+post['lng']+'" data-lat="'+post['lat']+'">'+post['name']+'</div>'
						+'</li>'
					);
					default_lat += post['lat']*1;
					default_lng += post['lng']*1;
					...
					default_item_content += '<a target="_blank" href="'+post['page']+'"><img src="'+post['featuredImage']+'"></a>';
					default_item_content += '</div>';
					default_item_content += '<div class="map_selected_item_text">';
					default_item_content += '<div class="map_selected_item_number">/'+post['number']+'</div>';
					default_item_content += '<h3 class="map_selected_item_title"><a target="_blank" href="'+post['page']+'">'+post['name']+'</a></h3>';
					...
					if('address' in post){
						...
						default_item_content += '<li>'+post['address']+town+'</li>';
					}
					if('phone' in post){
						var phone2 = '';
						if(post['phone_second'] != ''){
							phone2 = '<br>'+post['phone_second'];
						}
						default_item_content += '<li>'+post['phone']+phone2+'</li>';
					}
					if('email' in post){
						var email2 = '';
						if(post['email_second'] != ''){
							email2 = '<br><a href="mailto:'+post['email_second']+'">'+post['email_second']+'</a>';
						}
						default_item_content += '<li><a href="mailto:'+post['email']+'">'+post['email']+'</a>'+email2+'</li>';
					}
					default_item_content += '</ul>';
					default_item_content += '</div>';
					default_item_content += '<div class="map_selected_item_plus"><a target="_blank"  href="'+post['page']+'"></a></div>';
					default_item_content += '</div>';
