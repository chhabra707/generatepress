    ...
    $.fn.menumaker = function(options) {
        ...
        return this.each(function() {
            ...
            multiTg = function() {
                ...
                cssmenu.find(".has-sub").children('a').on('click', function(event) {
                    if($(this).contents().addBack().is(event.target)){
                        event.preventDefault();
                    }
                    $(this).parent().toggleClass('submenu-opened');
                    if ($(this).parent().children('ul').hasClass('open')) {
                        $(this).parent().children('ul').removeClass('open').stop().slideUp();
                    }
                    else {
                        $(this).parent().children('ul').addClass('open').stop().slideDown();
                    }
                });
            };
