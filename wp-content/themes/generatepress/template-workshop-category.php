                        ...
                        <div id="category_research" class="clear_both">
                            <form action="/ateliers-activites/" method="get" class="clear_both">
                                <div class="form_block select technic"> 
										...
										<?php 
										$terms = get_terms( 'ateliers-activites_cat', array(
												'hide_empty' => false,
												'parent' => 0  
											) );
										
										if ( ! empty( $terms ) && ! is_wp_error( $terms ) ){  foreach ( $terms as $term ) {		
												$term_children = get_term_children( $term->term_id, 'ateliers-activites_cat' ) ; 
												$taxonomy_name = 'ateliers-activites_cat';
												var_dump($term);
						...
						<?php
							if(isset($_GET['category_select'])){
								...
								$posts = get_posts(array(
									'post_type' => 'ateliers-activites',
									'numberposts' => -1,
									'tax_query' => array(
									array(
									  'taxonomy' => 'ateliers-activites_cat',
									  'field' => 'id',
