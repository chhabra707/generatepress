...
get_header(); 
	...
	/************Prepare form data fields default and variables**************/
	$actionUrl = home_url( $wp->request );
	$postType  = array('saveur');
	$taxonomy  = array('saveur_cat');	
  ...
  <div class="wrapper">
    <div id="category_research" class="clear_both"> 
      <form id="searchForm" action="<?php echo $actionUrl; ?>" method="post" class="clear_both">
		...
		<?php endforeach; ?>
		<?php wp_nonce_field( 'all_saveur_filter', 'allSaveur' ); ?>			  
...
?>									
	...
	//$postType = "'" . implode ( "', '", $postType ) . "'";
		...
		if (isset( $_POST['allSaveur'] ) &&  wp_verify_nonce( $_POST['allSaveur'], 'all_saveur_filter' ) ):
      ...
      <div class="items">					
        <a class="fixunderline" href="<?php echo get_the_permalink(); ?>">  						
          <div class="item_title">
            <p>
              <?php echo get_field('artist_town'); ?> 
            </p>
          </div>						
